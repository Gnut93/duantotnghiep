...
const RegisterPage = () => {
  ...
  const checkEmail = debounce(async () => {
    ...
    if (emailValue) {
      const response = await fetch(`http://localhost:4000/users/check-email/${emailValue}`);
    ...
    <div className="login-wrapper">
      <div className="form-container">
        ...
        <form className="form" onSubmit={handleSubmit} encType="multipart/form-data">
          <input ref={name} type="text" className="input" placeholder="Họ tên" />
          <input ref={email} type="email" className="input" placeholder="Email" onChange={checkEmail} />
          {isExistingEmail && <p className="error-message">Email đã tồn tại!</p>}
