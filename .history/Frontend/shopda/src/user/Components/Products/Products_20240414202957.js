...
const Products = () => {
  ...
  const handleSortChange = (event) => {
    ...
    // Cập nhật trạng thái `sortOption` với tùy chọn sắp xếp mới
  ...
  function PhanTrang({ pageSize }) {
    ...
    // Sắp xếp danh sách sản phẩm dựa trên tùy chọn sắp xếp
    const sortedSP = [...sp];
    if (sortOption === "date") {
  ...
  return (
    <>
      ...
      <div className="app-container">
        <h2 className="title">
          Danh mục: {loai ? loai["name"] : "Danh mục không xác định"}
        </h2>
        <PhanTrang pageSize={8} />
      </div>
    </>
  );
};
