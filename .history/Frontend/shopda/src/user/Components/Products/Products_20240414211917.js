...
const Products = () => {
  ...
  // Bỏ useEffect cũ trong component PhanTrang để không cần lắng nghe thay đổi của sortOption nữa

// Thay đổi lại component PhanTrang để sắp xếp và hiển thị danh sách sản phẩm theo sortOption

function PhanTrang({ pageSize }) {
  const [fromIndex, setFromIndex] = useState(0);

  // Sắp xếp danh sách sản phẩm dựa trên sortOption
  const sortedSP = useMemo(() => {
      const sortedList = [...sp];
      if (sortOption === "date") {
          sortedList.sort((a, b) => new Date(b.date) - new Date(a.date));
      } else if (sortOption === "price") {
          sortedList.sort((a, b) => a.price_sale - b.price_sale);
